...
class Game extends UI{
 ...
 #cells = [];
 #cellsElements = null;
 #cellsToReveal = 0; // lb pól do odkrycia
 #revealedCells = 0; // lb odkrytych pól
 ...
 #newGame(
  ...
  this.#isGameFinished = false; // restart gry po jej zakończeniu
  this.#revealedCells = 0; // restart lb odkrytych pól po zakończeniu gry
  ...
  this.#modal.infoText = this.#timer.numberOfSeconds < this.#timer.maxNumberOfSeconds ? `You won, it took you ${this.#timer.numberOfSeconds} second, congratulations!` : 'It took you a long time, but you did it, congratulations!';
 ...
 #handleCellClick = (e) => { //funkcja strzałkowa, żeby nie zgubić kontekstu
 ...
 #handleCellContextMenu=(e)=>{//funkcja strzałkowa, żeby nie zgubić kontekstu
  e.preventDefault();
  const target = e.target;
  const rowIndex = parseInt(target.getAttribute('data-y'), 10);
  const colIndex = parseInt(target.getAttribute('data-x'), 10)
